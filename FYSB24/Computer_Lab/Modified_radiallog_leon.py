...
Lagt till argumenten N, a:
N: antalet elektroner i atomen 
a: random variabel (se part 2 instructions)

ändringar i koden jämfört med original:
...
"""
def radiallog(l, n, Z, N, a, plot=True):
    ...
    while nodes != nodes_count or abs(dE) > 1.0e-10:
        ...
        while 1:
            ...
            U[1:] = -Z/r[1:] + (N-1)*r[1:]/(a**2 + r[1:]**2) + l*(l+1)/(2*r[1:]**2)
        ...
        # V(r) = ((-Z/r) + (N-1)*r/(a**2 + r**2))

        g = -2*r**2*(E -((-Z/r) + (N-1)*r/(a**2 + r**2))) + (l+1/2)**2;               #g function for Numerow
